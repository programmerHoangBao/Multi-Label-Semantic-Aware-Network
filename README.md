...
- **Type**: Multi-head self-attention with 4 attention heads
- **Embedding Dimension**: 1024 (matches Bi-LSTM output)
- **Purpose**: Learns weighted importance of different tokens in the sequence
- **Mechanism**: Identifies which parts of the input are most relevant for each label prediction
- **Output**: [batch_size, seq_len, 1024]
...
4. **Attention Mechanism**: Multi-head attention identifies important tokens and re-weights their contributions
...
- ✅ **Attention Mechanism**: Learns which tokens are important for each prediction
